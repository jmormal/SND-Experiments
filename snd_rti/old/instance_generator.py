    ...
    for j_plant in plants.values():
        ...
        if len(other_zone) > 0:
            ...
            op = (
                ow / ow.sum()
                if ow.sum() > 0
                else np.ones(len(other_zone)) / len(other_zone)
            )
        else:
            op = None
        if len(same_zone) > 0:
            ...
            sp = (
                sw / sw.sum()
                if sw.sum() > 0
                else np.ones(len(same_zone)) / len(same_zone)
            )
        else:
            sp = None
        suppliers = []
        ...
        for s in suppliers:
            ...
            for p in prods:
                ...
                inlay_frac[p] = (
                    round(rng.uniform(0, PARAMS["r_max"]), 3)
                    if route_has_inlay
                    else 0.0
                )
...
def main():
    ...
    print(
        f"  |V|={len(plants)}  |E|={len(routes)}  |R|={inst.n_rti_types}  |P|={inst.n_products}"
    )
