...
PARAMS = {
    ...
    "sigma_d": 0.8,
    "r_max": 0.3,
    "p_inlay": 0.15,  # automotive: many containers have nets/foam/dunnage that prevent folding on return
...
def generate_instance(size: str, seed: int) -> Network:
    ...
    for i in range(n_products):
        ...
        products[name] = Product(name=name, category=cat)
        cat_counters[cat] += 1
    ...
    for pi, prod in enumerate(product_list):
        ...
        inlay = 0
        # Register product on each compatible RTI
        for rid in compat:
            # Automotive: 1 engine block per stillage … 120 brackets per KLT
            cap = round(rng.uniform(1, 120), 1)
            if inlay_bool:
                inlay = round(rng.uniform(0.10, 0.25), 3)
            rtis[rid].add_product(prod, cap, inlay)
    ...
    for z in range(n_zones):
        ...
        for _ in range(n_hubs):
            ...
            h = Hub(
                ...
                y=round(hy, 2),
                zone_id=z,
                fixed_cost=round(rng.uniform(*PARAMS["fhc_range"]), 2),
                variable_cost=round(rng.uniform(*PARAMS["vhc_range"]), 3),
            )
            hubs[hid] = h
            zone_hubs.append(h)
            hid += 1
    ...
    for dest in all_plants:
        ...
        for src in suppliers:
            ...
            # Cap total demand so route is feasible at tightest mode/freq
            v_max_tmp = 0.8
            cap = PARAMS["delta_ftl"] / (PARAMS["q_min_ftl"] * v_max_tmp)
            total_d = sum(demand.values())
            if total_d > cap:
                scale = cap / total_d
                demand = {p: max(1.0, round(v * scale)) for p, v in demand.items()}
            ...
            rti_ids_for_edge: set[int] = set()
            for pi in prod_indices:
                rti_ids_for_edge.update(product_rti_compat[pi])
            allowed_empties = [rtis[rid] for rid in sorted(rti_ids_for_edge)]
